...
import org.sandholm.max.juttele.utils.MircColors;
import org.sandholm.max.juttele.utils.Smilies;
...
public class Message
{
    ...
    /*own message*/
    public static final int TYPE_OWN_MESSAGE = 2;
    ...
    public SpannableString render(Context context)
    {
        ...
        if (canvas == null) {
            ...
            SpannableString renderedText = new SpannableString("");
            ...
            canvas = new SpannableString(renderedText);
     ...
     * @return
     */
    public SpannableString renderSender(Context context)
    {
        Settings settings = new Settings(context);
        ...
        canvas = new SpannableString(canvas);

        if (hasSender()) {
            int start = (prefix + timestamp).length();
     ...
     * @return
     */
    public LinearLayout renderTextView(Context context, MessageListView msgListView)
    {
        ...
        if (getType() == TYPE_OWN_MESSAGE) {
            canvas = (LinearLayout) LayoutInflater.from(context).inflate(R.layout.ownmessage, msgListView, false);
        }
        else if (getType() == TYPE_MISC) {
            canvas = (LinearLayout) LayoutInflater.from(context).inflate(R.layout.statusmessage, msgListView, false);
        }
        else {
            canvas = (LinearLayout) LayoutInflater.from(context).inflate(R.layout.messagewithsender, msgListView, false);
        }

        messageView = (TextView) canvas.findViewById(R.id.message_contents);

        messageView.setAutoLinkMask(Linkify.ALL);
        messageView.setLinksClickable(true);
        messageView.setLinkTextColor(COLOR_BLUE);

        messageView.setText(this.render(context));
        messageView.setTextSize(settings.getFontSize());
        if (getType() == TYPE_MISC) {
            messageView.setTextSize(settings.getFontSize()-2);
        }
        messageView.setTextColor(COLOR_DEFAULT);

        if (hasSender()) {
            senderView = (TextView) canvas.findViewById(R.id.sender_nick);
            senderView.setText(this.renderSender(context));
            senderView.setTextSize(settings.getFontSize());
            senderView.setTextColor(COLOR_DEFAULT);
            senderView.setVisibility(View.VISIBLE);
        }

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
            setupViewForHoneycombAndLater(messageView);
        }
        return canvas;
    }
